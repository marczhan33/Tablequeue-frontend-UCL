...
// Party size wait times for restaurants
export const partySizeWaitTimes = pgTable("party_size_wait_times", {
  id: serial("id").primaryKey(),
  restaurantId: integer("restaurant_id").notNull(),
  partySize: text("party_size").notNull(), // "1-2 people", "3-4 people", "5+ people", "Any size"
...
export const insertPartySizeWaitTimeSchema = createInsertSchema(partySizeWaitTimes).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type InsertPartySizeWaitTime = z.infer<typeof insertPartySizeWaitTimeSchema>;
export type PartySizeWaitTime = typeof partySizeWaitTimes.$inferSelect;
