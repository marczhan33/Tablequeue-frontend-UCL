...
export default function DigitalQueueButton({ restaurantId, className }: DigitalQueueButtonProps) {
  ...
  return (
    <Button 
      className={`bg-red-500 hover:bg-red-600 text-white flex items-center justify-center gap-3 shadow-md ${className || ''}`}
      onClick={() => setLocation(`/restaurants/${restaurantId}/remote-waitlist`)}
    >
      <svg className="h-6 w-6 text-white flex-shrink-0" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
